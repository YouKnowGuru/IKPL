        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 max-w-6xl mx-auto">
            ...
            <main className="lg:col-span-9">
                ...
                <div className="space-y-6">
                  {sections.map(({ id, icon: Icon, title, content: text, color, bg }) => (
                    <div
                      key={id}
                      id={id}
                      className={`group p-8 rounded-3xl ${bg} border border-transparent hover:border-agro-green/20 transition-all duration-500 card-hover`}
                    >
                      <div className="flex items-start gap-5">
                        <div className={`w-12 h-12 bg-gradient-to-br ${color} rounded-2xl flex items-center justify-center flex-shrink-0 shadow-md group-hover:scale-110 group-hover:rotate-6 transition-transform duration-300`}>
                          <Icon className="h-6 w-6 text-white" />
                        </div>
                        <div>
                          <h2 className="font-display font-bold text-xl mb-3">{title}</h2>
                          <p className="text-zinc-600 dark:text-zinc-300 leading-relaxed">{text}</p>
                        </div>
                      </div>
                    </div>
                  ))}
              ...
              {/* Footer note */}
              <div className="mt-10 p-6 rounded-3xl bg-agro-green/5 border border-agro-green/15">
                <div className="flex items-start gap-4">
                  <Shield className="h-6 w-6 text-agro-green flex-shrink-0 mt-0.5" />
                  <div>
